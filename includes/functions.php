...
if ( ! function_exists( 'img' ) )
{
	function img( $src, $align = '', $comment = true, $thumbnail = true )
	{
		...
		if ( isset( $info['extension'] ) && $extension = strtolower( $info['extension'] ) )
		{
			...
			{
				return '<figure class="' . $align . ' img-thumbnail"><video class=img-responsive controls src="' . $uri . r( $src ) . '" preload=none></video>' . $img_source . '</figure>' . $n;
			}
		}
	}
}
...
if ( filter_has_var( INPUT_GET, 'page' ) && ! is_numeric( $get_page ) )
{
	...
	elseif ( $dl && $get_page === $download_contents )
	{
		...
		$breadcrumb .= '<li class=active>' . $download_contents . '</li>';
			...
			for( $i = 0, $c = count( $sections_in_categ_page ); $i < $c; ++$i )
			{
				...
				if ( is_dir( $default_imgs_dir = $article_dir . $s . 'images' ) && ! is_link( $default_imgs_dir ) )
				{
					...
					if ( $glob_default_imgs )
					{
						...
						{
							$default_image = '<a href="' . $url . $categ_link . $s . $title_link . '" class=thumbnails>' . $thumbnail_left . '</a> ' . $n;
						}
	...
	if ( is_dir( $current_article_dir = 'contents' . $s . $get_categ . $s . $get_title ) && ! is_link( $current_article_dir ) && is_file( $current_article = $current_article_dir . $s . 'index.html' ) && ! is_link( $current_article ) )
	{
			...
			if ( $glob_image_files )
			{
				...
				for( $i = 0, $c = count( $images_in_page ); $i < $c; ++$i )
				{
					...
					else
					{
						...
						{
							$article .= '<video class="thumbnail center-block img-responsive" controls src="' . $url . r( $images_in_page[$i] ) . '"></video>';
						}
					}
				}
				$article .= '</div>';
		...
		if ( $glob_prev_next )
		{
			...
			for( $i = 0, $c = count( $sort_prev_next ); $i < $c; ++$i )
			{
				...
				if ( $prev_next_parts[0] == $article_filemtime )
				{
					$prev_next_count = $prev_next_parts[2];
				}
		...
		if ( $use_comment && is_dir( $comment_dir ) && ! is_link( $comment_dir ) )
		{
			...
			{
				$article .= '<strong class=page-title>' . $comments_not_allow . '</strong>' . $n;
			}
		}
	}
	else
...
if ( $glob_info_files || $dl || $use_contact )
{
	...
	if ( $dl )
	{
		$aside .= '<a class="list-group-item' . ( $get_page === $download_contents ? ' list-group-item-info' : '' ) . '" href="' . $url . r( $download_contents ) . '">' . $download_contents . '</a>' . $n;
	}
	if ( $use_contact && $contact )
	{
		$aside .= '<a class="list-group-item' . ( $get_page === $contact_us ? ' list-group-item-info' : '' ) . '" href="' . $url . r( $contact_us ) . '">' . $contact_us . '</a>' . $n;
	}
	$aside .= '</div>' . $n;
}
...
function numlinks( $pagenum, $maxpage, $pages_visible, $scriptname = '' )
{
	...
	if ( $pagenum > 1 )
	{
		$article .= '<li><a href="' . page_name( ( $pagenum - 1 ), $scriptname ) . '">' . $nav_laquo . '</a></li>' . $n;
	}
	else
	{
	...
	while ( $i <= $pages_visible )
	{
		...
		{
			if ( $i == $pagenum )
				$article .= '<li class=active><a>' . $pagenum . '</a></li>' . $n;
			else
				$article .= '<li><a href="' . page_name( $i, $scriptname ) . '">' . ( $i ) . '</a></li>' . $n;
		}
		...
		else
		{
			if ( $i == ceil( $pages_visible / 2 ) )
				$article .= '<li class="disable active"><a>' . $pagenum . '</a></li>' . $n;
			else
			{
