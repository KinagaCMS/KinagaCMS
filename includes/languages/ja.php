...
#サイドボックスのタイトル
$sidebox_title = ['新着記事', 'インフォメーション', '良く読まれている記事', '最近のコメント', 'ログイン', 'こんにちは。%sさん', '関連記事', 'シェア', '引用リンク', '目次', 'カテゴリ', '最近のトピック'];
...
#プレースホルダー
#['0: 検索', '1: ログインメールアドレス', '2: お問い合わせ名前', '3: お問い合わせメール', '4: お問い合わせ内容', '5: アプローチメッセージ']
$placeholder = ['Search...', 'example@yourmail.com', '', '', '', '一言メッセージ（必須ではありません）'];
...
#ログイン
$login = 'ログイン';
$login_message = ['ログインにはメールアドレスが必要となります。', 'メールアドレスを入力し、ログイン・チケットを取得して下さい。'];
...
$approach_info = [
	...
	'%sさんからアプローチを受けています。<a class=alert-link href="'. $url. '?user=%s#approval">'. $myprof. '</a>を確認して下さい。'
	];

$users_info = 'ユーザー情報の確認とログインの可否を設定できます。';
...
$approach_form_message = [
	...
	'%sさんからの承認前に下のボタンを押せば連絡通知を取り下げることができます。承認された場合には送信フォームが表示されます。',
	'送信に失敗しました。',
...
$user_not_found_title = 'ユーザーが見つからないかアカウントが凍結されています';
$​ask_admin = '問題が解決しない場合は、<a class=text-danger href="'. $url. $contact_us. '">こちら</a>からお問い合わせ下さい。';

$forum_form = ['返信', 'トピック作成', 'トピックタイトル', 'スレッド作成', 'スレッドタイトル', '残り%s文字'];
$forum_disallow_symbols = ['"','#','$','%','&','\'','(',')','*','+',',','-','.','/',':',';','>','<','=','?','[','\\',']','^','_','`','{','|','}','~'];
$forum_replace_symbols = ['”','＃','＄','％','＆','’','（','）','＊','＋','，','－','．','／','：','；','＞','＜','＝','？','［','￥','］','＾','＿','｀','｛','｜','｝','～'];
$forum_guests = [
	$forum. 'からURLのお知らせ',
