...
if ( is_file( $config = '..' . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'config.php' ) && !is_link( $config ) ) include_once $config;
...
$sort = array_filter( $sort );
	...
	foreach( $sort as $image ) {
		...
		if ( array_search( $formats, $extensions ) !== false ) {
			...
			'<small class=text-uppercase>' . date( $time_format, $img[0] ) . ' ' . size_unit( filesize( $img[1] ) ) . ' ' . $formats . '</small>';

			if ( $formats == 'mp4' || $formats == 'ogg' || $formats == 'webm' ) {

				$href =

				'<video class="img-thumbnail img-responsive individual-thumbnail" controls src="' . $uri . '"></video>';
				...
				'<img class="img-thumbnail img-responsive individual-thumbnail" src="' . $uri . '" alt="' . $basename . '">';
	...
	for( $i = 0, $c = count( $imgs_in_page ); $i < $c; ++$i ) echo $imgs_in_page[$i]; echo

	'</div>' . $n;

		if ( $count_imgs > $number_of_imgs ) { echo
			...
			if ( $page > 1 ) echo
				...
				'<li class=previous><a href="' . $url . '?p=' . ( $page - 1 ) . '"><span class="glyphicon glyphicon-menu-left"></span> ' . $imgs_prev . '</a></li>' . $n;

			if ( $page < ceil( $count_imgs / $number_of_imgs ) ) echo

				'<li class=next><a href="' . $url . '?p=' . ( $page + 1 ) . '">' . $imgs_next . ' <span class="glyphicon glyphicon-menu-right"></span></a></li>' . $n;
