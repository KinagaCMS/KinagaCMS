...
if ($glob = glob('{contents/[!!]*/[!!]*/index.html,contents/[!!]*.html}', GLOB_BRACE + GLOB_NOSORT))
{
	...
	foreach ($glob as $files)
	{
		...
		if ('.' !== $categ)
			echo '<url><loc>', $url, r($categ. '/'. $title), '</loc><lastmod>', $filetime, '</lastmod></url>';
		elseif ('contents/index.html' === $files)
			echo '<url><loc>', $url, '</loc><lastmod>', $filetime, '</lastmod></url>';
		elseif (is_file('contents/'. $page. '.html'))
			echo '<url><loc>', $url, r($page), '</loc><lastmod>', $filetime, '</lastmod></url>';
	}
}
if (is_dir('downloads'))
	echo '<url><loc>', $url, r($download_contents), '</loc></url>';
if ($use_contact && $mail_address)
	echo '<url><loc>', $url, r($contact_us), '</loc></url>';
echo '</urlset>';
