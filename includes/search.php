...
if (false !== $pos && $query)
{
	...
	$header .= '<title>'. $result_title. ' - '. ($pages > 1 ? sprintf($page_prefix, (int)$pages). ' - ' : ''). $site_name. '</title>'. $n;
	$article .= '<header><h1 class="'. $h1_title[0]. '">'. $result_title. '</h1></header>'. $n;
	...
	if ($glob_search)
	{
		...
		if (isset($outputs))
		{
			...
			foreach ($results_in_page as $results)
			{
				...
				$article .= '<section class="'. $results_wrapper_class. ' position-relative">'. $n;
		...
		$no_results = true;
	if ($no_results) $article .= '<section class="alert alert-warning">'. $no_results_found. '</section>'. $n;
}
elseif (false !== $fpos && $fquery)
{
