...
.avatar,.avatar img{height:100px;object-fit:cover;width:100px}
...
.flow li:before{counter-increment:num;content:counter(num,upper-roman);height:30px;width:30px;line-height:30px;border:thin solid lightgray;display:block;margin:0 auto 10px auto;border-radius:50%;background-color:#16161c}
...
.card,.list-group-item:not(.list-group-item-primary):not(.list-group-item-secondary):not(.list-group-item-success):not(.list-group-item-danger):not(.list-group-item-warning):not(.list-group-item-info):not(.list-group-item-light):not(.list-group-item-dark),.page-item,.page-link,.nav-tabs .nav-item.show .nav-link,.nav-link.active,#side>div[id],.input-group-text,.form-control,.article,#article-nav,#main #toc,#side a:hover:not(.btn),.bg-inherit,#main>header,entry{background-color:'. ($color ? $c05 : 'rgba(0,0,0,.1)'). '}
#main>header{line-height:2.5;margin-bottom:2em;padding:3em;text-align:center}
#side>div[id]{margin:.75rem;width:30%}
...
echo '@media(max-width:767px){.card-columns{column-count:1}#form .input-group-text{display:block}#side>div[id]{width:100%}}';
