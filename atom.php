...
$atom_files = glob( '{' . $glob_dir . 'index.html,contents' . $s . '*.html}', GLOB_BRACE + GLOB_NOSORT );
...
if ( $atom_files ) {
	...
	for( $i = 0, $c = count( $sort ); $i < $c && $i < $number_of_feeds; ++$i ) {
		...
		if ( file_exists( $atom_imgs_dir = $atom_article_dir . $s . 'images' ) && is_dir( $atom_imgs_dir ) && !is_link( $atom_imgs_dir ) ) {
			...
			$glob_atom_imgs = glob( $atom_imgs_dir . $s . '*', GLOB_NOSORT );
		...
		if ( file_exists( $atom_background_imgs_dir = $atom_article_dir . $s . 'background-images' ) && is_dir( $atom_background_imgs_dir ) && !is_link( $atom_background_imgs_dir ) ) {

			$glob_atom_background_imgs = glob( $atom_background_imgs_dir . $s . '*', GLOB_NOSORT );
	...
	'<p>' . description( $atom_section ) . ' <a href="' . $id . '">' . $more_link_text . '</a></p>' . $n . $atom_image . $atom_background_image .
	'</div>' . $n .
	'</content>' . $n .
