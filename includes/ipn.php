...
if (is_dir($article_dir = 'contents/'. $categ. '/'. $title)) $article_url = $url. r($categ. '/'. $title); else exit;
...
if (filter_has_var(INPUT_POST, 'custom'))
{
	...
	while (!feof($fp))
	{
		...
		if (0 === strcmp($res, 'VERIFIED'))
		{
			...
			elseif (filter_has_var(INPUT_GET, 'n') && is_dir($buyer_dir))
			{
				if ('1' === filter_input(INPUT_GET, 'n'))
				{
					file_put_contents($buyer_txt, $n. $separator, FILE_APPEND);
					$payer_subject = sprintf($shop_mail_subject[4], $site_name);
					$payer_body = sprintf($shop_mail_body[4], handle($buyer_dir. '/prof/'), $url. r($categ. '/'. $title)). $n. $n.
					$separator. $n. $site_name. $n. $url;
					$seller_subject = sprintf($shop_mail_subject[5], $site_name);
					$seller_body = sprintf($shop_mail_body[5], handle($buyer_dir. '/prof/'), $item_name, $url. '?user='. str_rot13($user)).
					$n. $n;
				}
				else
				{
					file_put_contents($buyer_dir. '/'. md5($buyer), $item_name);
					$payer_subject = sprintf($shop_mail_subject[6], $site_name);
					$payer_body = sprintf($shop_mail_body[6], handle($buyer_dir. '/prof/'), $item_name). $n. $n.
					$separator. $n. $site_name. $n. $url;
					$seller_subject = sprintf($shop_mail_subject[7], $site_name);
					$seller_body = sprintf($shop_mail_body[7], handle($buyer_dir. '/prof/'), $item_name, $url. '?user='. str_rot13($user)).
					$n. $n;
				}
			}
			else
			...
			$mail_header = $mime. 'From: '. $site_name. '<'. $mail_address. '>'. $n. 'Content-Type: text/plain; charset='. $encoding. $n. 'Content-Transfer-Encoding: 8bit'. $n;
			if (mail(dec($buyer), $payer_subject, $payer_body, $mail_header))
				mail($mail_address, $seller_subject, $seller_body, $mail_header);
			#else file_put_contents($tmpdir. '1.txt', $mail_address. $n. $seller_subject. $n. $seller_body. $n. $mail_header);
		}
	}
	fclose($fp);
}
