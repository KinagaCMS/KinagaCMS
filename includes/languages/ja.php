...
#ペイパル決済用メールアドレス（空欄の場合は $mail_address を使用）
$business_mail_address = '';
#ペイパル決済テスト用メールアドレス（テスト時以外は空欄またはコメントアウト）
$sandbox_mail_address = '';
#ペイパル決済通貨コード
$currency_code = 'JPY';
$locale_code = 'JP';
#税率（空欄またはコメントアウトで非適用）
...
#送料（空欄またはコメントアウトで非適用）
$shipping = '500';
#配送時間指定（空欄またはコメントアウトで非適用）
$delivery_times = ['時間指定なし', '午前中', '12時頃～14時頃', '14時頃～16時頃', '16時頃～18時頃', '18時頃～20時頃', '19時頃～21時頃', '20時頃～21時頃'];
$price_format = '価格：%d円（送料%d円＋税込）';
$price_short_format = '価格：%d円（税込）';
$item_total = '残り：%d';
$paypal_logo = 'https://www.paypalobjects.com/marketing/web/jp/developer/logos-buttons/download/319x110_c_20200120.png';
...
#サイドボックスのタイトル
$sidebox_title = ['新着記事', 'インフォメーション', '良く読まれている記事', '最近のコメント', 'ログイン', 'こんにちは。%sさん', '関連記事', 'シェア', '引用リンク', '目次', $category = 'カテゴリ', '最近のトピック', '購読日数は%sです'];
...
$btn = ['訂正', '送信', '保存', '拒否', '削除', '承認', '掲載', '編集', 'プレビュー', '続きを読む…', '今すぐ購入', '売り切れ', 'ログインして購入', '閉じる', '新規作成'];

$intervals = ['y' => '年前', 'm' => 'ヶ月前', 'w' => '週間前', 'd' => '日前', 'h' => '時間前', 'i' => '分前', 's' => '秒前'];

#ログイン
$login = 'ログイン';
$login_message = ['ログインにはメールアドレスが必要となります。', 'メールアドレス宛に「ログイン・チケット」を送付します。', 'ログイン成功', 'もう一度ログインする際は、ログイン・チケットを取得して下さい。', '<br>購読期間が終了しました。引き続きご購読をご希望の際は、該当ページにてご購入手続きを行って下さい。'];
...
$admin_menus = [
	...
	'記事編集', '移動先', '同名のフォルダまたはファイルが存在します', '要ログインのメッセージまたは購読用コメント記号等の表記。空欄は記事要約'
	];

$shopping_info = [
	'ショッピングについて',
	'本サイトのショッピング機能にはペイパル決済を使用しておりますので、本サイト内にカード情報等個人情報が保存されることはありません',
	'お買い上げありがとうございました',
	'しばらくしても本サイトから自動送信メールが届かない場合は、以下のボタンよりお問い合わせ下さい。',
];

$shop_mail_subject = [
	'ご購読手続き完了のお知らせ（自動送信メール） - %s',
	'購読受付のお知らせ（自動送信メール） - %s',
	'ご注文受付完了のお知らせ（自動送信メール） - %s',
	'注文受付のお知らせ（自動送信メール） - %s',
];
...
[
	'%s 様'. $n. $n.
	'この度は、%s の有料記事をご購読頂きありがとうございました。'. $n.
	'購読料%d円のご入金を承っておりますのでログインしてご覧下さい。'. $n.
	'購読期間は%s（%sまで）となっております。',

	'%s に %s 様から購読の注文がありました。'. $n.
	$separator. $n.
	'期間：%s'. $n.
	'価格：%d円'. $n.
	$separator. $n,
	...
	'%s様'. $n. $n.
	'この度は、%sのオンラインショップをご利用頂きありがとうございました。'. $n.
	'%s (%d円)のご入金を承りましたので、「%s」の指定で配送手続きを致します。'. $n.
	'発送手続きが完了しましたら、後ほどその旨をご案内させて頂きます。',

	'%1$sに%2$s様から注文がありました。'. $n.
	$separator. $n.
	'商品名　：%3$s'. $n.
	'数量　　：%4$d'. $n.
	'価格　　：%5$d'. $n.
	'発送先　：〒%10$s %9$s %8$s %7$s %6$s'. $n.
	'配送時間：%11$s'. $n.
	$separator. $n,
];
